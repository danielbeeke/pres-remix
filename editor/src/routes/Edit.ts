  ...
  thumbnails (context) {
    return html`
      <div class="thumbnails">
        ...
        ${State.presentation['presentation:slides'] ? State.presentation['presentation:slides'].map((slide) => html`
          <button class=${`thumbnail ${context.params.slide === lastPart(slide['@id']) ? 'active' : ''}`} onclick=${() => goTo(`/slide/${lastPart(slide['@id'])}`)}>${slideThumbnail({
